...
void on_peer_read(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf) {
	...
	free(buf->base);	//fang: here free buffer alloced in on_alloc_buffer(), why we hold a buffer for send but malloc and free for recv ??
}
...
void on_peer_connected(uv_stream_t* server_stream, int status) {
	...
	if (uv_accept(server_stream, (uv_stream_t*)client) == 0) {
		...
		client->data = peerstate;	//fang: all client's data field point to peerstate.
