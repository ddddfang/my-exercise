...
fd_status_t on_peer_ready_send(int sockfd) {
	...
	if (nsent == -1) {
		if (errno == EAGAIN || errno == EWOULDBLOCK) {
			return fd_status_W;	//send seems failed , so we need send again!
		} else {
			perror_die("send");
		}
	}
	if (nsent < sendlen) {
		peerstate->sendptr += nsent;	//continue send before all data sends out.
		return fd_status_W;
	} else {
			...
			if (FD_ISSET(fd, &readfds)) {
				...
				if (fd == listener_sockfd) {
					...
					if (newsockfd < 0) {
						...
						if (newsockfd > fdset_max) {	//need update fdset_max
						...
						//once connect, report peer connection, and server need send a ack, so we want send!
