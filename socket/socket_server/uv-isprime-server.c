...
void on_work_completed(uv_work_t* req, int status) {
	...
	int rc;
	if ((rc = uv_write(writereq, (uv_stream_t*)peerstate->client, &writebuf, 1, on_sent_response)) < 0) {
...
void on_peer_read(uv_stream_t* client, ssize_t nread, const uv_buf_t* buf) {
	if (nread < 0) {
		...
		if (mode && !strcmp(mode, "BLOCK")) {
			...
			// Otherwise, compute isprime on the work queue, without blocking the callback.
...
int main(int argc, const char** argv) {
	...
	if ((rc = uv_listen((uv_stream_t*)&server, N_BACKLOG, on_peer_connected)) < 0) {
